...
public class TblBaseServiceOracle {
	private static final Logger logger = Logger.getLogger(TblBaseServiceOracle.class);
...
logger.debug(strBuf.toString());
	...
	public static ArrayList<TblBase> queryByCondition(DbBase db, SingleTblQueryCondition stqc) throws Exception {
		...
		try{
			...
			strBuf.append(" rownum <= " + stqc.getPc().getEndRow());
...
logger.debug("sql=" + strBuf.toString());
	...
	public static HashMap<String,ArrayList<TblBase>> queryByCondition(DbBase db, MultiTblQueryCondition mtqc) throws Exception {
		...
		try{
			...
			strBuf.append("select rownum rn, ");
			...
			strBuf.append(" rownum <= " + mtqc.getPc().getEndRow());
	...
	public static int getCountByCondition(DbBase db, SingleTblQueryCondition stqc) throws Exception {
		...
		try{
			...
			if(stqc.getLimit() > 0){
				...
				strBuf.append(" rownum <= " + stqc.getLimit());
			}
...
logger.debug(strBuf.toString());
	...
	public static int getNextSequenceValue(DbBase db, TblBase tbl) throws Exception {
		...
		try {
			...
			String strSql = "SELECT " + tbl.getSequenceName() + ".nextval FROM DUAL";
	...
	public static String getCreateTblSql(TblBase tbl) throws Exception {
		...
		for(int i=0;i<fieldList.size();i++){
			...
			strBuf.append(fieldList.get(i));
			strBuf.append("\t\t");
			strBuf.append(TblBaseUtil.getOracleColumnTypeDef(tbl.getFieldType(i), tbl.getFieldSize(i)));
