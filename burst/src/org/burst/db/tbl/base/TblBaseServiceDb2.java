...
public class TblBaseServiceDb2 {
	private static final Logger logger = Logger.getLogger(TblBaseServiceDb2.class);
	...
	public static int create(DbBase db, TblBase tbl) throws Exception {
		...
		try{
			int id = getNextSequenceValue(db, tbl);
			tbl.setInt("id", id);
...
logger.debug(strBuf.toString());
	...
	public static ArrayList<TblBase> queryByCondition(DbBase db, SingleTblQueryCondition stqc) throws Exception {
		...
		try{
			...
			//add close for page controller select
			strBuf.append(") where rn >= " + stqc.getPc().getStartRow());
			strBuf.append(" and rn <= " + stqc.getPc().getEndRow());
...
logger.debug("sql=" + strBuf.toString());
	...
	public static HashMap<String,ArrayList<TblBase>> queryByCondition(DbBase db, MultiTblQueryCondition mtqc) throws Exception {
		...
		try{
			...
			strBuf.append("select * from (");
			//for select with rownum
			strBuf.append("select rownumber() over(");
			//sort
			...
			//end rownum
			strBuf.append(") where rn >= " + mtqc.getPc().getStartRow());
			strBuf.append(" and rn <= " + mtqc.getPc().getEndRow());
...
logger.debug(strBuf.toString());
	...
	public static int getNextSequenceValue(DbBase db, TblBase tbl) throws Exception {
		...
		try {
			...
			if (tbl.getSequenceName() == null) {
				throw new Exception("Sequence Name is null");
			}

			String strSql = "SELECT nextval for " + tbl.getSequenceName() + " from sysibm.sysdummy1";
			rs = db.getResultSet(strSql);
			if (rs.next()) {
				return rs.getInt(1);
			} else {
				throw new Exception("getSequenceNextValue failed");
			}
		} catch (Exception e) {
			throw e;
		} finally {
	...
	public static String getCreateTblSql(TblBase tbl) throws Exception {
		...
		for(int i=0;i<fieldList.size();i++){
			...
			strBuf.append(fieldList.get(i));
			strBuf.append("\t\t");
			strBuf.append(TblBaseUtil.getDb2ColumnTypeDef(tbl.getFieldType(i), tbl.getFieldSize(i)));
			
			if(i==0){
				strBuf.append(" not null primary key ");
			}
		...
		if(DbConfig.usePartition() && tbl.isPartition()){
			...
			for(int i=0;i<DbConfig.getPartitionCount();i++){
				lvalue = DbConfig.getPartitionSize() * i;
				
				strBuf.append(" partition " + TblBaseUtil.PARTITION_MARK + i);
				
				if(i == 0){
					strBuf.append(" starting from (minvalue) inclusive");
				}else{
					strBuf.append(" starting from (" + lvalue);
					strBuf.append(") inclusive");
				}
				
				
				if(i<DbConfig.getPartitionCount()-1){
					strBuf.append(" ending at (" + (lvalue + DbConfig.getPartitionSize()));
					strBuf.append(") exclusive");
				}else{
					strBuf.append(" ending at (maxvalue) exclusive");
				}
				
				//to multi tablespaces
	...
	public static String getCreateSequenceSql(TblBase tbl) throws Exception {
		...
		strBuf.append(tbl.getSequenceName());
		strBuf.append(" as integer start with 1 increment by 1 NOMAXVALUE NOCYCLE CACHE 24");
