...
public class TblBaseServiceSqlserver {
	private static final Logger logger = Logger.getLogger(TblBaseServiceSqlserver.class);
...
logger.debug(strBuf.toString());
	...
	public static ArrayList<TblBase> queryByCondition(DbBase db, SingleTblQueryCondition stqc) throws Exception {
		...
		try{
			...
			strBuf.append(tblCondition.getTblName());
			strBuf.append(".*, row_number() over(");
			//sort
			//sql server row_number must has order by
			if(stqc.getSortFieldList().size() > 0){
				strBuf.append(SortField.getSortString(stqc.getSortFieldList()));
			}else{
				strBuf.append(" order by ");
				strBuf.append(tblCondition.getFieldList().get(0));
				strBuf.append(" asc ");
			}
			...
			strBuf.append(") as tblOutPageController where rn >= " + stqc.getPc().getStartRow());
...
logger.debug("sql=" + strBuf.toString());
	...
	public static HashMap<String,ArrayList<TblBase>> queryByCondition(DbBase db, MultiTblQueryCondition mtqc) throws Exception {
		...
		try{
			...
			strBuf.append("select * from (");
			//for select with rownum
			strBuf.append("select row_number() over(");
			//sort
			//sql server row_number must has order by
			if(mtqc.getSortFieldList().size() > 0){
				...
				strBuf.append(".");
				strBuf.append(mtqc.getTblList().get(0).getFieldList().get(0));
				strBuf.append(" asc ");
			}
			strBuf.append(") as rn,");
			...
			//end rownum
			//sql server must use table alias name
			strBuf.append(") as tblOutPageController where rn >= " + mtqc.getPc().getStartRow());
...
logger.debug(strBuf.toString());
	...
	public static String getCreateTblSql(TblBase tbl) throws Exception {
		...
		for(int i=0;i<fieldList.size();i++){
			...
			strBuf.append(fieldList.get(i));
			strBuf.append("\t\t");
			strBuf.append(TblBaseUtil.getSqlserverColumnTypeDef(tbl.getFieldType(i), tbl.getFieldSize(i)));
			
			if(i==0){
				strBuf.append(" identity(1,1) primary key not null ");
			}
		...
		/*if(SystemUtil.getDbTableSpace() != null && SystemUtil.getDbTableSpace().length() > 0){
			strBuf.append(" tablespace ");
			strBuf.append(SystemUtil.getDbTableSpace());
		}*/
	...
	public static int getLastInsertId(DbBase db, TblBase tbl) throws Exception {
		ResultSet rs = null;
		try {
			String strSql = "SELECT IDENT_CURRENT('" + tbl.getTblName() + "') id" ;
